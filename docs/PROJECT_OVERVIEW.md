...
- **本文の再生成(`service.regenerate_engage_draft` / `POST /drafts/{id}/regenerate`)**: **kind は変えず**本文だけを同じ型でリサーチ付きに作り直す導線(型を変えるのは recast)。未承認(DRAFT)の REPLY/QUOTE のみ。Inbox の「リプ案を再生成」「引用案を再生成」ボタンから呼ぶ。recast と同じくジョブ化(`{"job_id"}` を即返す)。DRAFT 以外/POST 種別は `PolicyViolation`(ジョブの error)。
...
| POST | `/drafts/{id}/regenerate` | 絡みの本文再生成: **kind は変えず**同じ型でリサーチ付きに本文だけ作り直す(ボディ不要)。未承認のREPLY/QUOTEのみ。**ジョブ化: `{"job_id"}` を即返す**(404のみ同期。PolicyViolation はジョブの error) | 404 |
