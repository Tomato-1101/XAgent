...
- **内蔵 BackgroundScheduler**: API プロセスの `lifespan` 内で `init_db()` 後、`scheduler_enabled` が真なら APScheduler の `BackgroundScheduler(timezone="UTC")` を生成。別デーモンを起こさず同1プロセスで予約投稿(`queue_tick`、既定60秒)・絡み案生成(`monitor_tick`、既定180秒)・ニュース速報生成(`news_tick`、既定600秒)・有名人ウォッチ(`celeb_tick`、既定600秒)・バズウォッチ(`buzz_tick`、既定600秒)を回す。**ただし monitor は `MonitorSettings.auto_monitor_enabled`、news は `NewsSettings.auto_news_enabled`、celeb は `MonitorSettings.celeb_watch_enabled`、buzz は `MonitorSettings.buzz_watch_enabled`(いずれも既定OFF)でゲートされ、OFFの間は即returnしてAPIを消費しない**(UIトグルでオンオフ。乱造防止のため既定OFF＝必要な時だけオンにする)。launchd 常駐構成では実際に回るのはこの内蔵スケジューラ。
...
| `buzz_watch_enabled` | `bool` | `False` | バズウォッチ(buzz_tick)を回すか。UI(Settings)のトグルで制御 |
| `buzz_min_faves` | `int` | `3000` | バズ判定のいいね数しきい値(min_faves検索の値) |
...
- **バズウォッチ(`run_buzz_once` / buzz_tick)**: アカウントを問わず**「既にバズった投稿」を網羅的に拾って**絡み案を作るストリーム(ゴール: バズ投稿のリプ欄に自分のリプライがある確率を上げる)。**バズの予測はしない**: X検索の `min_faves:` 演算子で「いいねが `buzz_min_faves`(既定3000)以上付いた」という**実績だけ**を条件に `min_faves:{n} lang:ja within_time:6h -filter:replies` の**検索1クエリ/回**(Latest順・最大100件)で検出する。**since_id カーソルは使わない**(バズ投稿は作成後しばらくして閾値を超えるため、作成ID基準のカーソルでは「後から閾値を超えた投稿」を全て取りこぼす)。重複防止は既存Draft除外(状態問わず)のみ。min_impressions は適用不要(min_favesで既に足切り済み)。どれに絡むかはAI選定に委ね、候補の `note` で「公式アカウントの告知・ファンダム向け定常投稿・部外者のリプが浮く話題は選ばない」と指示。1回の生成上限は `BUZZ_MAX_PER_TICK=3`。**自動tickのみ乱造ガード**: 承認待ち(DRAFT)の絡み案が `BUZZ_BACKLOG_MAX=30` 件以上なら生成をスキップ(手動実行はガードを通らない)。自動実行は `buzz_watch_enabled`(既定OFF)でゲート、手動は Settings「今すぐチェック」/ `POST /monitor/buzz-run-once`(ジョブ化)。
- **AIバッチ選定(`collect_engage_candidates` → `formatter.select_engagements`)**: 全有効ソースの直近24時間の候補を**1回のバッチAI判断**にまとめて渡し、「どの投稿に絡むか(分散)」「reply / quote のどちらか」「本文(叩き台)」までを一度に生成する(従来の直列スキャン＋投稿ごと判断は先頭アカウントがバジェットを使い切り同一アカウント偏りを生んだため廃止)。**同一アカウントは原則1件・最大2件**(プロンプト指示＋コード側でも3件目以降を破棄)。**選定はツール無効(構造化出力・候補最大120件のため検索を入れない)。本文はここでは叩き台で、確定した kind ごとに後段の本文生成(リサーチ付き)で作り直す**(`monitor._research_engage_body`)。選定結果は `service.create_engage_draft_from_text` で LLM を呼ばずそのまま Draft 化(選定理由を `source_text` に `[AI選定理由]` として保持)。`run_once` の総生成数バジェット(`max_drafts_per_run`)が選定の `max_n` になる。メンション(自分宛)は従来通り個別生成・引用RTせず常に reply。
...
- **返信/引用生成(generate_reply / generate_quote・リサーチ付き)**: 相手投稿から AI が本文生成。**`target_tweet_id` を受け取り元投稿URL(`https://x.com/{handle}/status/{id}`、handle欠落時は `…/i/web/status/{id}`)をプロンプトに添え、WebSearch/WebFetch で話題・固有名詞・最新の文脈を調べてから本文を書く**(誤解・的外れ回避用。知識はひけらかさない)。ツールは `Formatter._research_complete`(既定は `run_claude(..., allowed_tools=["WebSearch","WebFetch"], timeout=claude_cli_research_timeout_seconds=480)`)経由で許可。テストは `complete` 注入のみのとき `complete` を流用(`research_complete` 未注入のフォールバック)。長さ帯は kind で出し分け(`_length_hint("reply")`=短帯 `_REPLY_LENGTH_BANDS`/`_length_hint("quote")`=`_QUOTE_LENGTH_BANDS`)、`random.choice` で毎回同じ長さに寄るのを防ぐ。**reply は「短く簡単」を最優先(6〜45字帯。素の共感/一言ツッコミ)、quote は視点/一次情報を一段添える(20〜120字帯)**。常に1セグメント(分割しない)、140字厳守。両関数は `user_prompt`(UI で入れる今回限りの方向性指示・既定 "")を受け取り、非空なら system 先頭に `## 追加指示(ユーザー指定・最優先)` ブロックとして差し込む(`_user_direction_block`、空なら何も足さない=従来のお任せ生成)。
- **型の事後切替(`service.recast_engage_draft` / `POST /drafts/{id}/recast`)**: AI が選んだ型を人が上書きする導線。未承認(DRAFT)の REPLY⇄QUOTE 間でのみ、新しい型で本文を(リサーチ付きで)再生成し `kind` を差し替える(元ポスト情報・`target_tweet_id` は維持)。Inbox の「引用RTで送る」「手動で返信に切替」ボタンから呼ぶ。DRAFT 以外/POST 種別は `PolicyViolation`(API は 409)。
- **本文の再生成(`service.regenerate_engage_draft` / `POST /drafts/{id}/regenerate`)**: **kind は変えず**本文だけを同じ型でリサーチ付きに作り直す導線(型を変えるのは recast)。未承認(DRAFT)の REPLY/QUOTE のみ。Inbox の「リプ案を再生成」「引用案を再生成」ボタンから呼ぶ。各カードの「再生成の方向性」入力(任意)が body `{user_prompt?}` として渡り `generate_*` に反映される(空ならお任せ・body省略可)。recast と同じくジョブ化(`{"job_id"}` を即返す)。DRAFT 以外/POST 種別は `PolicyViolation`(ジョブの error)。
...
| POST | `/compose/quote-from-url` | ツイートURLから引用案(引用RT)をAI生成(`create_quote_drafts`)。body `{url, user_prompt?, count=1}`(方向性指示は任意・count は service で1〜5にクランプ)。Inboxの手動ボタン。**ジョブ化: `{"job_id"}` を即返し結果は `GET /jobs/{id}`(result は `{"drafts":[...]}`)**(URL不正のみ同期400) | 使用 | 400 |

### jobs(`/jobs`) — 長時間AI生成の非同期実行(2026-06-11〜)

生成系(quote-from-url / monitor run-once / recast / regenerate)は Claude CLI 待ちで数十秒〜15分かかり、同期のままだと**ブラウザ fetch の上限(Chrome 約300秒)・CLIタイムアウト(240秒)・worker 再起動(--reload/kickstart)**のどれかに当たって「TypeError: Failed to fetch」になっていた。これらは `{"job_id"}` を即返し、フロント(`api.ts` の `runJob`)が `GET /jobs/{job_id}` を2秒間隔でポーリングする。リサーチ付き本文生成(generate_reply/quote)はウェブ検索ターンで既定240秒を超えうるため `claude_cli_research_timeout_seconds=480` を別途使う。

| メソッド | パス | 用途 | エラー |
|---|---|---|---|
| GET | `/jobs` | **実行中**ジョブの一覧 `[{job_id, label, progress, elapsed_seconds}]`。label はジョブ種別(`monitor-run-once`/`quote-from-url`/`recast`/`regenerate`/`news-run-once`/`celeb-run-once`/`buzz-run-once`)。画面を開き直した時の再接続用 | - |
...
| POST | `/drafts/{id}/regenerate` | 絡みの本文再生成: **kind は変えず**同じ型でリサーチ付きに本文だけ作り直す。body `{user_prompt?}`(方向性指示・省略可)。未承認のREPLY/QUOTEのみ。**ジョブ化: `{"job_id"}` を即返す**(404のみ同期。PolicyViolation はジョブの error) | 404 |
...
| POST | `/monitor/buzz-run-once` | バズウォッチを1回実行(`run_buzz_once`)。`?limit=N` でその回の生成上限を絞る。**ジョブ化**(label=`buzz-run-once`)。result は `{candidates, reply_suggestions, quote_suggestions}`。手動実行は乱造ガード(承認待ち件数)を通らない |
| GET | `/monitor/settings` | 監視設定取得 |
| PUT | `/monitor/settings` | 監視設定更新(None 以外のフラグのみ部分更新。`min_impressions`/`celeb_watch_enabled`/`celeb_list_id`/`buzz_watch_enabled`/`buzz_min_faves` 含む) |
...
| **Inbox** | メンション返信案・絡み案の承認/編集/送信 | `kind !== "post"` のみ表示し、性質が違うので**返信案(sky)と引用案/引用RT(violet)に分けて表示**(`kind==="reply"`/`"quote"`)。「監視を1回実行」(横の件数入力でその回の生成上限を指定・既定10、`monitorRunOnce(limit)`)。**「URLから引用案を作る」手動ボタン**(URL入力＋方向性プロンプト(任意の Textarea)＋パターン数(1〜5・既定1)→`POST /compose/quote-from-url`→`service.create_quote_drafts` で相手投稿からAIが引用コメントを count 件生成し引用案セクションに追加。方向性が空ならお任せ生成。自分のコメントを添える引用は Compose の指令フロー)。**返信案はAPI送信不可のため「コピーして元ポストを開く」(返信文をクリップボードにコピー＋相手ポストを別タブで開く=ポップアップブロック回避でwindow.open先行)＋「却下」のみ**(注記を上部に表示)。承認導線は無く DRAFT のまま手動送信。引用案は「承認して送信」(BlackoutGate経由、`approve→postNow` 順)/「承認のみ」/「却下」。両セクションとも未承認の絡み案には**型切替ボタン(「引用RTで送る」/「手動で返信に切替」=`recast`)と「リプ案を再生成」/「引用案を再生成」(`regenerateDraft`、kindは変えず本文だけリサーチ付きで作り直す。カードごとの「再生成の方向性」入力=`regenPrompt[id]` を任意で渡せる)**を併置し、実行中は `actionProgress`(リサーチ進捗)を Spinner 付きで表示 |
...
3. **バッチAI選定(`formatter.select_engagements`)**: 候補一覧を1回のLLM呼び出し(JSONスキーマ強制・**ツール無効**)に渡し、「どの投稿に絡むか(**同一アカウントは原則1件・最大2件**=分散)」「reply / quote のどちらか(迷えば reply)」「本文(140字厳守・**叩き台**)」「選定理由」までまとめて生成する。コード側でも検証(候補に無いID破棄・140字超破棄・kind正規化・同一アカウント3件目以降破棄・`max_n`=残バジェットで打ち切り)。
4. **本文のリサーチ再生成(`monitor._research_engage_body`)**: 選定された各案を、確定した kind ごとに `formatter.generate_reply/generate_quote`(WebSearch/WebFetch 付き・`target_tweet_id` 渡し)で本文を作り直す(元投稿の文脈を調べ、誤解・的外れを避けてから書く)。**例外/タイムアウト時は select_engagements の叩き台 `s["text"]` にフォールバック**(候補収集に費やしたAPIを無駄にせず・乱造ガードを維持)。進捗は「リサーチして本文作成中 i/n件」で通知(無表示の長待ち回避)。run_once/run_celeb_once/run_buzz_once の3経路で共通利用。
5. **Draft化(`service.create_engage_draft_from_text`)**: 本文は生成済みなので**LLMを呼ばずに** `status=DRAFT` の下書きを作る。選定理由は `source_text` に `[AI選定理由] ...` として保持(Inboxの承認判断材料)。元投稿のエンゲージ指標(`target_view_count`/`target_like_count`/`target_retweet_count`=候補の view/like/retweet)も保存し、DraftCard でインプレッション等を表示する。型を変えたいときは Inbox の切替ボタン=`recast_engage_draft`、本文だけ作り直すなら「リプ案を再生成」=`regenerate_engage_draft`。
...
### バズウォッチ(`monitor.run_buzz_once`)

`run_buzz_once(session, x_client, formatter, max_drafts=None, progress=None) -> {"candidates", "reply_suggestions", "quote_suggestions"}`。アカウント不問で「既にバズった投稿」に絡み案を作る(狙いと検索式は§6)。`run_celeb_once` との差分:

- **クエリは1本固定**: `min_faves:{buzz_min_faves} lang:ja within_time:{BUZZ_WINDOW_HOURS=6}h -filter:replies`。リストもメンバー展開も無い。
- **since_id カーソルを使わない**: バズ投稿は作成後しばらくして閾値を超えるため、作成ID基準の since_id では「後から閾値を超えた投稿」を構造的に全て取りこぼす(celeb と逆の設計判断)。Latest順×`search_recent` の100件上限があるので、実効的な検出窓は直近数時間。選定落ちした投稿は次tickで再評価される(エンゲージがさらに伸びて選ばれ直すことを許す)。
- **乱造ガードは daemon 側**: `buzz_tick` が `engage_backlog_count`(DRAFT状態のreply/quote件数) >= `BUZZ_BACKLOG_MAX=30` でスキップ。手動(`POST /monitor/buzz-run-once`)はガードを通らない。
- 1回の生成上限は `max_drafts or BUZZ_MAX_PER_TICK=3`。コスト記録 note は `buzz-select`。
...
設定の二重制御: 投稿の緊急停止は `POSTING_ENABLED`、絡み案の自動生成のオンオフは `MonitorSettings.auto_monitor_enabled`、ニュース速報の自動生成は `NewsSettings.auto_news_enabled`、有名人ウォッチは `MonitorSettings.celeb_watch_enabled`、バズウォッチは `MonitorSettings.buzz_watch_enabled`(いずれも既定OFF・UIトグル)。1日上限は `MAX_POSTS_PER_DAY`(自然)と `HARD_CAP_POSTS_PER_DAY`(ハード)の二段。
